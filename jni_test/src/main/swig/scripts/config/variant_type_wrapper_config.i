...
%define type_expand(target_type, c_t, nice_t)
    target_type(const c_t& value) : m_original(value) {}
...
%typemap(javapackage) target_type, std::shared_ptr<target_type> "$typemap(javapackage, original_type)";
%typemap(javaimports) target_type, std::shared_ptr<target_type> %{
