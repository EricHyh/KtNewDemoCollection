...
%define %shared_variant_type_bridge(original_type, c_target_type, j_target_type)
...
%define %normal_variant_type_bridge(original_type, c_target_type, j_target_type)
...
//C++层，JNI函数参数，java类型转C++类型
%typemap(in) original_type %{
c_target_type* jarg$argnum_element = (c_target_type*)$input;
if (jarg$argnum_element) {
  $1 = jarg$argnum_element->m_original;
}
%}
...
//C++层，JNI函数返回值，C++类型转java类型
%typemap(out) original_type %{
*(c_target_type **)&$result = new c_target_type($1);
%}
%typemap(out) original_type& %{
c_target_type* value_ptr = new c_target_type(*$1);
*(c_target_type **)&$result = new c_target_type(*$1);
...
%typemap(directorout, descriptor="L$packagepath/$typemap(jstype, original_type);") original_type %{
if ($input) {
  $1 = ((c_target_type*)$input)->m_original;
}
%}
%typemap(directorout, descriptor="L$packagepath/$typemap(jstype, original_type);") original_type& %{
#error "typemaps for $1_type not available"
