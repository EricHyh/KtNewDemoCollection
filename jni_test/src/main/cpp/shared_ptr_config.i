...
%define %director_shared_ptr_wrapper(TYPE, DESCRIPTOR_PACKAGE)


%typemap(directorout, descriptor="L"#DESCRIPTOR_PACKAGE";") std::shared_ptr<TYPE> %{
    if (!$input) {
        $1 = nullptr;
    } else {
        std::shared_ptr<TYPE> * jresult_smartPtr = *(std::shared_ptr<TYPE> **)&$input;
        auto *jresult_ptr = dynamic_cast<SwigDirector_##TYPE*>(jresult_smartPtr->get());
